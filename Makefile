...
race:
	VM_NAME="$(VM_NAME)" ./scripts/vm-exec.sh bash ./scripts/guest/race.sh "$(STEP)" "$(RACE_CPU)" "$(RACE_CPU_SECONDS)"

team-race:
	VM_NAME="$(VM_NAME)" ./scripts/vm-exec.sh bash ./scripts/guest/team-race.sh "$(STEP)" "$(RACE_CPU)" "$(TEAM_A_MEMBERS)"
