...
[観測結果を説明する](../handson/conclusion.md)で触れた「長いスライスを保ちながら、起床タスクへ早く CPU を渡す」方針では、待っているタスクを記録する状態と、実行中のタスクに CPU を譲らせる仕組みが必要になる。
`tick` でその状態を周期的に確認する設計も考えられるが、起床した瞬間に反応する処理とはタイミングが異なる。
