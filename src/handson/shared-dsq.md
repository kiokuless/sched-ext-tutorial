...
三つと一つの二列に分けて列を交互に選ぶと、三つのタスクは自分たちの列に来た順番を分け合い、もう一方の一つは、その列の順番を独占できる。
各タスクに同じスライスを割り当てても、列の分け方によって CPU 時間に差が付く。

この分類を実装するため、BPF 側からカーネルへ二つの待ち行列の作成を依頼する。
...
独自 DSQ を使う場合は、自作コードで取り出す列を選び、ローカル DSQ へ移す関数を呼ぶ必要がある。
そこで、スケジューラの初期化時に呼ばれる **`init`** に DSQ の作成を、CPU が次に実行するタスクを求めたときに呼ばれる **`dispatch`** に取り出しを実装する。

編集先は `lab/src/bpf/main.bpf.c` である。
loader が動いていれば、先に終了する。
前の課題で加えた変更を別のファイルなどへ保存してから、`make restore STEP=03` で開始状態を揃える。
この状態では名前によるスライスの変更はなく、すべてのタスクに同じ 10 ミリ秒を渡す。
...
ロードする前に、以降の手順で新しく登録した二つの関数を定義し、`oreore_enqueue` の投入先も変更する。

## oreore_init で二つの DSQ を作成する

`oreore_init` で、タスクを投入する前に二つの DSQ を作成する。
作成時と投入時に同じ DSQ を指定できるよう、それぞれの ID に名前を付ける。
...
どちらか一方でも作成に失敗した場合は、エラー値をカーネルへ返し、必要な DSQ が揃わないままスケジューラが動き始めることを防ぐ。

DSQ の作成にはメモリ確保を伴うため、途中で待機できる実行コンテキストの `init` に置く。
`BPF_STRUCT_OPS_SLEEPABLE` は、そのような callback を定義するためのマクロである。
...
プロセス単位で分類するため、番号には `p->tgid` を使う。
Linux のタスクはスレッドに対応し、`p->pid` はスレッド自身の番号、`p->tgid` はそのタスクが属するプロセスの番号を表す。
`pid` で分けると同じプロセスのスレッドが別の列に入る可能性があるが、`tgid` なら同じ列に揃えられる。[^task-id]

偶数なら `EVEN_DSQ`、奇数なら `ODD_DSQ` を、`scx_bpf_dsq_insert()` の第2引数に指定する。
`slice_ns` と `enq_flags` は完成例 `STEP=03` の値を使い、投入先だけを変更する。

すべてのタスクをこの分類に通すため、`oreore_select_cpu` は CPU の候補を返す実装を保つ。
`select_cpu` からローカル DSQ へ直接投入すると `enqueue` が省略され、偶奇の振り分けを通らなくなる。
idle CPU が見つかった場合も、投入は `enqueue` に任せる。[^scheduling-cycle]
...
移動できた列の反対を次の候補として保存するので、両方に実行できるタスクがあれば 0、1、0、1 と選び、片方が空ならタスクがある側から取り出せる。

次の候補の読み書きには、`bpf_map_lookup_elem()` が返す、呼び出し元 CPU のキー 0 の値へのポインタを使う。
取得に失敗した場合を `if (!turn)` で確認してから参照し、DSQ の ID は `^ 1` で 0 と 1 を切り替える。
...
周期タスクが一つだけでも完了すれば、片方の列にしかタスクがない場合に実行が進むことを確認できる。
二つの列へ交互に順番が回ることは、この出力からは分からない。
...
まず、全員に同じ10ミリ秒のスライスを渡す一つの FIFO で測る。
...
偶数側の各プロセスは三回に一回しか自分の列の先頭になれないため、列へ同じ回数だけ順番を回しても、プロセスごとの CPU 時間は等しくならない。
独自 DSQ の分類条件と取り出す順番によって、誰と CPU 時間を分け合うかを指定できる。
