...
その処理を書くのが `lab/src/bpf/main.bpf.c` である。
Mac 側のエディタで開き、カーネルがどの関数を呼び、タスクがどの待ち行列を通って実行されるかをコードと対応させる。
前章の最後まで進めた状態は、`checkpoints/step-03-short-slice/src/bpf/main.bpf.c` と同じである。

## callback をカーネルへ登録する

`main.bpf.c` の関数は、Linux カーネルがスケジューリングの途中で呼ぶ callback である。
この C ファイルには `main()` がなく、ファイル末尾の `SCX_OPS_DEFINE` で、呼ばれる場面と関数を対応付ける。
...
たとえば `.enqueue = (void *)oreore_enqueue` は、タスクを列へ入れる場面で `oreore_enqueue` を呼ぶという指定である。
...
`timeout_ms = 5000` は、実行可能なのに長く動けないタスクを検出するためのタイムアウトを5秒に設定している。
...
`BPF_STRUCT_OPS` は、カーネルから呼び出せる BPF プログラムとして callback を定義するためのマクロである。[^callback-definition]
最初の引数に関数名を置き、それに続けてカーネルから受け取る引数を宣言する。
カーネルは、対象タスクを `p`、前回動いていた CPU を `prev_cpu`、wakeup に関する条件を `wake_flags` に渡す。

この実装は、CPU 選びをカーネルの `scx_bpf_select_cpu_dfl()` に任せ、その CPU 番号を返す。
この関数は、タスクが使える CPU の中から空いているものを探す。
選ばれた CPU が空いている状態（**idle**）かどうかは `is_idle` で受け取るが、今の実装では使わない。
候補を返した後、タスクを実行待ちの列へ入れるのは `oreore_enqueue` の役割である。

ただし、使える CPU が一つだけなら、候補を選ぶ必要がないため `select_cpu` は省略される。
前章の `taskset -c 0` がこの場合に当たり、タスクの投入は `enqueue` が担当する。[^cpu-selection]
後の[偶数用と奇数用に振り分ける](./shared-dsq.md#oreore_enqueue-で偶数用と奇数用に振り分ける)でも、この分担を保ち、タスクの分類を `enqueue` に加える。

## oreore_enqueue で実行待ちの列に入れる

`oreore_enqueue` は、実行可能なタスクを待ち行列へ入れ、次の順番に使うスライスを割り当てる。
このように、実行を待つタスクを列へ入れる処理を **`enqueue`** と呼ぶ。
今のコードでは、wakeup 後や、スライスを使い切ってほかのタスクと交代するときにカーネルが呼ぶ。

スライスを使い切ったタスクは引き続き実行可能なので、再び列に入って次の順番を待つ。
一方、I/O などの待機に入ったタスクは、待ちが解消されるまで実行できない。
この場合は待機に入る時点では列に戻さず、wakeup したときに `enqueue` で戻す。
...
`scx_bpf_dsq_insert()` は、カーネルから受け取ったタスク `p` を指定の DSQ へ入れる関数である。
ここでは、複数の CPU で共有する組み込みの **グローバル DSQ**（`SCX_DSQ_GLOBAL`）に投入する。
スライスには、前章で変更した `slice_ns` の 10 ミリ秒を指定している。
...
`slice_ns` を変えると各タスクに割り当てる CPU 時間が変わり、同じ列で待つタスクへ順番が回るまでの時間にも影響する。

### 列から取り出すのはカーネル

グローバル DSQ からタスクを取り出す処理はカーネルが持っているため、この BPF ファイルには書かない。
各 CPU には、次に実行するタスクを置く **ローカル DSQ** があり、それが空なら、カーネルはグローバル DSQ からその CPU で実行できるタスクを移す。
...
カーネルは、BPF スケジューラを取り外す際に `oreore_exit` を呼び、引数の `ei` に終了理由を渡す。
loader の停止で接続を解放した場合も、カーネルが異常を検出して停止した場合も、この関数で理由を受け取る。
通知の単位はスケジューラ全体であり、個々のタスクが終了するたびに呼ばれる関数ではない。
...
`UEI_RECORD` が `ei` の内容をその保存先へ書き込み、Rust の loader が読み取って端末へ表示する。
